...
const NewDesignProject = ({ popupDetails, modalIsOn, setmodalIsOn }) => {
  ...
  const sendEmail = (e) => {
    ...
    if (message === "" || name === "") {
          ...
          toast.success(Rate +' rating has been sent successfully for this project! 🦄', {
            theme: "dark",
          });
          setname('');
          setemail('');
  ...
  return (
    <div className='fixed top-0 overflow-y-scroll  z-[10000000] max-h-[100vh] pb-[400px] bg-[var(--bg)] w-screen overflow-x-hidden'>
          ...
          <div className="px-[32px] py-[24px] bg-[#13253A] rounded-lg mt-[10px]">
            ...
            <div className="colours flex items-center justify-center gap-[5px] flex-wrap">
              {popupDetails[0].colorPallete.map((e) => {
                ...
                return <div className='colorBox text-white md:w-[40%] lg:w-[30%] w-[49%] flex-wrap' style={{backgroundColor: e}} key={e}> {e}</div>
              })}
      ...
      <div className="research px-[32px] py-[24px] bg-[#13253A] rounded-lg md:mx-[100px] mx-[10px]">
        <h4 className='text-[var(--secondary)]  text-[24px] font-mono md:text-left text-center md:mb-[10px] mb-[32px]'>{popupDetails[0].ResearchAnalysisTitle}</h4>
        <div className="images w-[100%] flex justify-start items-center gap-[10px] flex-wrap mt-[24px] text-white md:py-[50px]">
          {popupDetails[0].researchImgs.map((e) => {
            return <div className='researchbox  flex-col bg-none mb-[50px] md:mb-[10px]' key={e.img}>
              <img
            src={e.img}
      ...
      {/* PROJECT SNAPSHOTS */}
      <div className="research px-[32px] py-[24px] bg-[#13253A] rounded-lg md:mx-[100px] mx-[10px] md:mt-[80px] mt-[20px]">
      ...
      <div className="Ratebox px-[32px] py-[24px] bg-[#13253A] rounded-lg md:mx-[100px] mx-[10px] mt-[24px] flex md:justify-between justify-center items-center flex-wrap">
        ...
        <form action="post" className='flex items-center justify-center flex-wrap gap-3' ref={forms} onSubmit={sendEmail}>
          <input name="user_name" placeholder='Your name' required autoComplete='off' onChange={(e) => setname(e.target.value)}   className='md:w-[40%] m-[0] py-[16px] rounded-sm' value={name}/>
          {/* <label htmlFor="email">Email Address</label> */}
          {/* <label htmlFor="email">subject</label> */}
          <input type="text" name='subject' placeholder='Subject' typeof='text' value={subject} required autoComplete='off'readOnly onSubmit={(e)=>setsubject ("A new Project Rating - Title: "+popupDetails[0].title + "!!!!")} className='hidden'/>

          {/* <label htmlFor="message">Enter Message</label> */}
          <input name="user_message" id="message" value={message} cols="50" rows="10" placeholder='Enter Message' className='text-[var(--secondary)] hidden bg-[var(--bg2)] p-[20px]' readOnly  required onSubmit={(e)=>setmessage(e.value)} ></input>
          <select name="rate" id="rate" className='px-[32px]  rounded-sm py-[16px] outline-none mr-2' onChange={(e)=>setRate(e.target.value)} required >
            <option value="Bleh" >Bleh</option>
            <option value="Not Bad" >Not Bad</option>
            <option value="Awesome">Awesome</option>
            <option value="Blow My Mind">Blow My Mind</option>
          </select>
          <button type="submit" className='px-[32px]  rounded-sm py-[16px] bg-[var(--secondary)] text-[var(--bg)] outline-none  ' onClick={() => {
            let messageMe = `${name} just Rate your  ${popupDetails[0].title} project. He rated your project: ${Rate}`
            setmessage(messageMe)
            setsubject("A new Project Rating - Title: "+popupDetails[0].title + "!!!!")
            setemail("osbornefashe@gmail.com")
          }}>Submit</button>
        </form>
      </div>
      ...
      <button type="submit" className='md:px-[32px]  md:w-auto w-[92%] mx-[4%] rounded-sm md:py-[16px] mt-[16px]  py-[16px] bg-red-500 text-white outline-none   md:mx-[100px]' onClick={() => setmodalIsOn(!modalIsOn)}>Close This</button>
      <ToastContainer  />
    </div>
  )
}
