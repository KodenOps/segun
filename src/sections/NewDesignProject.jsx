...
const NewDesignProject = ({ popupDetails, modalIsOn, setmodalIsOn }) => {
	...
	const sendEmail = (e) => {
		if (message === '' || name === '') {
			...
			toast.success(
				Rate + ' rating has been sent successfully for this project! 🦄',
				{
					theme: 'dark',
				}
			);
		...
		<div className='fixed top-0 overflow-y-scroll  z-[10000000] max-h-[100vh] pb-[400px] bg-[var(--bg)] w-screen overflow-x-hidden'>
			...
			<div className=' flex md:flex-row flex-col md:px-[32px] px-[10px] py-[24px] md:mx-[70px]  gap-[10px] h-auto'>
						...
						<div className='bottomBox p-[12px] bg-[#29394c] w-[92%] mx-[4%] gap-5 md:gap-0 flex md:flex-row flex-col items-top justify-between rounded-lg'>
							...
							<div className='gettingStarted  text-white text-[14px] md:w-[50%] w-full'>
								<h4 className='text-[var(--secondary)] '>
									{popupDetails[0].gettingStartedTitle}
								</h4>
								<p className='font-mono md:w-[90%]'>
									{popupDetails[0].gettingStartedTexts}
								</p>
							</div>
							{/* right */}
							<div className='gettingStarted  text-white text-[14px] md:w-[50%] w-full'>
								<h4 className='text-[var(--secondary)] '>
									{popupDetails[0].snoopAroundTitle}
								</h4>
								<p className='font-mono'>
									{' '}
									{popupDetails[0].snoopAroundTexts}{' '}
								</p>
							</div>
						</div>
					) : (
				...
				<div className='right md:w-[45%]   text-white h-[100%]'>
					...
					<div className='solution  px-[32px] py-[24px] bg-[#13253A] rounded-lg'>
						...
						<p className='text-[16px] font-mono text-center mb-[16px] md:w-[80%] w-full'>
							After careful studies into the problems, Here are the solutions to
							the problem I came up with.
						</p>
			...
			{popupDetails[0].researchImgs ? (
				<div className='research px-[32px] py-[24px] bg-[#13253A] rounded-lg md:mx-[100px] mx-[10px]'>
					<h4 className='text-[var(--secondary)]  text-[24px] font-mono md:text-left text-center md:mb-[0px] mb-[32px]'>
					...
					<div className='images w-[100%] flex justify-start items-start gap-[10px] flex-wrap md:mt-0 mt-[24px] text-white md:py-[50px]'>
						{popupDetails[0].researchImgs.map((e) => {
							return (
								<div
									className='researchbox h-auto  flex-col bg-none mt-[20px] mb-[20px] md:mb-[30px]'
									key={e.img}>
									<img
										src={e.img}
										...
										key={e.name}
									/>
									<p className='mt-[16px] text-center md:text-md text-sm'>
										{e.name}
									</p>
								</div>
							);
			...
			<div className='research px-[32px] py-[24px] bg-[#13253A] rounded-lg md:mx-[100px] mx-[10px] md:mt-[20px] mt-[20px]'>
				...
				<p className='text-[16px] font-mono text-center md:text-left mb-[16px] md:w-[40%] my-[16px] text-white'>
					...
					showcase the final product. Click The Behance Btn to preview more
				</p>
				<div className='images w-[100%] flex justify-start items-center gap-[10px] flex-wrap mt-[80px] text-white '>
					{popupDetails[0].snapshotImg.map((e) => {
						return (
							<div
								className='researchbox  flex-col bg-none mb-[20px] md:mb-[50px] lg:mb-[100px]'
								key={e}>
								<img
									src={e}
									className='img1 w-full md:object-cover object-fit rounded-md '
				...
				<a
					href={popupDetails[0].link}
					target='_blank'
					rel='noreferrer'
					className='px-[32px] md:w-[300px] w-full rounded-md py-[16px] bg-[var(--secondary)] text-[var(--bg)] mt-[24px]'>
					{popupDetails[0].ctaName}
				</a>
			</div>
